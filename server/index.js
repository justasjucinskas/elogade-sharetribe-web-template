...
const { sitemapRoute, sitemapXmlRedirect } = require('./resources/sitemap');
const { getExtractors } = require('./importer');
const renderer = require('./renderer');
const dataLoader = require('./dataLoader');
const { localeMiddleware, stripLocaleFromResourcePath } = require('./localeMiddleware');
...
// Crawlers also request robots.txt and sitemaps under a locale prefix
// (/en/sitemap-index.xml). Serve them from the locale-free routes below.
app.use(stripLocaleFromResourcePath);
...
// /sitemap.xml has no document of its own: permanent redirect to the index.
app.get('/sitemap.xml', sitemapXmlRedirect);

// Handle different sitemap-* resources. E.g. /sitemap-index.xml
app.get('/sitemap-:resource', sitemapRoute);
